...
import { View, Text, TextInput, TouchableOpacity, Alert, SafeAreaView, ActivityIndicator, Image } from 'react-native';
...
const EditProfileScreen = () => {
    ...
    const pickImage = async (fromCamera: boolean) => {
        ...
        if (fromCamera) {
            ...
            result = await ImagePicker.launchCameraAsync({
                // VOLTAMOS PARA A VERSÃO ANTIGA E FUNCIONAL
    ...
    const handleUpdateProfile = async () => {
        try {
            ...
            const { data: { user } } = await supabase.auth.getUser();
            if (!user) throw new Error("Usuário não encontrado");
            const updates = { id: user.id, username, full_name: fullName, phone, updated_at: new Date() };
